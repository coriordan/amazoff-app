...
const CartTotal = ({items}) => {
  let total = items.reduce(
              (sum, i) => (sum += i.quantity * i.format.price.amount), 0);
  return (
  ...
  return (
    <li className="media cart-item my-4">
      ...
      <div className="media-body d-flex flex-row justify-content-between align-items-start">
        <div className="w-50">
    <h6 className="cart-item__title mt-0 mb-1">{item.title} ({item.format.name})</h6>
          <div className="cart-item__meta text-muted">by {item.author}</div>
        </div>
        <div className="cart-item__meta text-muted">{item.format.price.currency + ' ' + 
                                                       Number(item.format.price.amount).toFixed(2)}</div>
